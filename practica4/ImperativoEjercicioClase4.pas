...
a. Almacene los productos vendidos en una estructura eficiente para la búsqueda por código de producto. De cada producto deben quedar almacenados la cantidad total de unidades vendidas y el monto total. De cada venta se lee código de venta, código del producto vendido, cantidad de unidades vendidas y precio unitario. 
...
Program ImperativoEjercicioClase4;
...
de unidades vendidas y el monto total. }
  ...
  Begin
    ...
    else if (elem.codigoProducto < a^.dato.codigo) then InsertarElemento(a^.HI, elem)
...
begin
  writeln;  writeln ('----- Modulo B ----->');
  maxCant := -1;
  CodigoDeProductoConMayorCantidadDeUnidadesVendidas(a, maxCant, maxCod);
  if (maxCant = -1) then writeln('Arbol sin elementos')
  else begin
...
{ Contenga un módulo que reciba la estructura generada en el punto a y un código de producto y retorne la cantidad de códigos menores que él que hay en la estructura. }
  ...
  begin
    if (a = nil) then ObtenerCantidadCodigosMenores := 0
    else if (unCodigo < a^.dato.codigo) then begin
      ObtenerCantidadCodigosMenores := 1+ObtenerCantidadCodigosMenores(a^.HI, unCodigo)
    end;
  end;
...
comprendidos entre los dos valores recibidos (sin incluir)}
  ...
  begin
    ...
    else if (a^.dato.codigo >= codigo2) then ObtenerMontoTotalEntreDosCodigos := 0
